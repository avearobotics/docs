---
title: "I2RT adapter example"
description: "Wrap an I2RT-controlled arm with the ROS 2 topics Sentinel expects."
---

This example keeps the I2RT hardware loop in a small ROS 2 node. The node publishes measured `JointState` continuously and accepts named `JointTrajectory` commands from Sentinel.

The same pattern works for other Python SDKs: initialize the hardware once, publish measured state on a timer, and convert each incoming command into the SDK's native call.

<Warning>
  Test this node without Sentinel first. The subscriber commands physical hardware. Add the limits, watchdog, fault handling, and emergency-stop integration required by your robot before use.
</Warning>

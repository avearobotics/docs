...
This is **the primary way robots connect to Sentinel**: your robot speaks standard ROS 2 — a command topic in, a state topic out — and a config maps Sentinel onto your topics. No custom packages, no Sentinel code on your robot. If your hardware is on the [supported list](/hardware/supported), it's even less work: grab its ready-made config and [install and run](/installation).

This section covers what your robot publishes and subscribes to — the topics, message types, units, and rates — and ends with a [pre-filled config](/integration/configuration) you copy and edit. You can set everything up and test it on your own with plain ROS 2 tools before connecting anything.

Already running `ros2_control`? You're most of the way there: a stock forward position controller (or `joint_trajectory_controller`) plus your `/joint_states` stream is exactly the interface Sentinel drives.
...
<CardGroup cols={3}>
  <Card title="1 · Robot control interface" icon="robot" href="/integration/robot-adapter">
    Arm, gripper, and base. Receiving commands and reporting joint state, with units and rates.
  </Card>
  <Card title="2 · Camera interface" icon="video" href="/integration/camera-adapter">
    Streaming a camera feed to the headset over a standard image topic.
  </Card>
  <Card title="3 · Set up your config" icon="file-code" href="/integration/configuration">

...
The runtime and your controllers run on the same `ROS_DOMAIN_ID`, so discovery works without extra setup.
...
<Note>
  Sentinel reads your joint names from the URDF you give us, so the names you already use are the names in the messages. You don't rename or reorder anything to match Sentinel; we handle any mapping in your config, and the same applies to the `JointState` you publish back. If your names or order change, tell us and we update the config — no changes on your side.
</Note>
...
<Note>
  We pick the exact range and message type with you based on your gripper and put them in the config. Your controller reads the topic and drives the gripper.
</Note>
